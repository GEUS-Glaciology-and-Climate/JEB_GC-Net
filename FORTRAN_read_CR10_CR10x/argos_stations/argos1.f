...
c ---------------------------------------------------------	
	in1='/ice/tools/aws/temp/argos.in'
	out1='/ice/tools/aws/temp/04.1.dat'
	out2='/ice/tools/aws/temp/05.1.dat'
	out3='/ice/tools/aws/temp/07.1.dat'
	out4='/ice/tools/aws/temp/12.1.dat'
	out5='/ice/tools/aws/temp/14.1.dat'
	out6='/ice/tools/aws/temp/03.1.dat'
c ---------------------------------------------------------
	print*,'Begin Argos 1, converts binary to decimal.'
...
c        if(sta.eq.24739) nn=2
c        if(sta.eq.24740) nn=3
c        if(sta.eq.24741) nn=4
c        if(sta.eq.11182) nn=6  
c        if(sta.eq.01162) nn=7    
c ----------------------------------------- GITS
	if(sta.eq.24740)then
	   write(2,2001) (da(m),m=1,7),sta,(out(n),n=1,16)
	endif
c ----------------------------------------- NASA-U
	if(sta.eq.11181)then
	   write(7,2001) (da(m),m=1,7),sta,(out(n),n=1,16)
	endif
c ----------------------------------------- Humboldt
...
c ----------------------------------------- TUNU-N
c	if((sta.eq.111819).or.(01162))then
	if(sta.eq.01162)then
	   write(4,2001) (da(m),m=1,7),sta,(out(n),n=1,16)
	endif
c ----------------------------------------- NASA-E
	if(sta.eq.11182)then
	   write(5,2001) (da(m),m=1,7),sta,(out(n),n=1,16)
	endif
c ----------------------------------------- NGRIP
	if(sta.eq.24739)then
	   write(6,2001) (da(m),m=1,7),sta,(out(n),n=1,16)
	endif
